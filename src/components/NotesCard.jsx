    ...
    <div className="w-full gap-[10px] flex flex-col h-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <div className='border-[1.5px] border-customPurple w-[25px] h-[26px] rounded-[10px] flex items-center justify-center'>
      ...
      <div className='flex w-full h-full gap-2'>
        {notesData?.map((note, index) => (
          <div key={index} className='w-full border-[1px] border-customGrey p-[10px] flex flex-col justify-between rounded-[10px]'>
            <div className='flex flex-col gap-[8px]'>
              <h3 className='text-secondary text-[16px]'>{note.title}</h3>
              <ul className='text-customTextGray text-[12px]'>
              ...
              <div className='flex flex-col gap-2'>
                {note?.attachments?.length !== 0 && (
                  <div className='flex flex-col'>
                    <div className='text-secondary text-[14px] font-semibold'>Attachments</div>
                    <div className='flex flex-col gap-1'>
                      {note.attachments.slice(0, 2).map((attachment, attIndex) => (
                        <div key={attIndex} className='flex items-center gap-1'>
                          ...
                          <p className='text-[10px] text-customTextGray'>{attachment.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {note?.attachments?.length === 0 && (
                  <div className='flex items-center justify-center w-full'>
                    <p className='text-[12px] text-customTextGray'></p>
                  </div>
                )}
              </div>
            </div>
